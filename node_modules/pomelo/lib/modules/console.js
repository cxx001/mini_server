...
Module.prototype.monitorHandler = function(agent, msg, cb) {
  ...
  switch(msg.signal) {
    ...
    case 'restart':
      ...
      break;
    case 'reload':
	  this.app.reload(msg.hotFile, function (hotModuleCache) {
		utils.invokeCallback(cb, {
			serverId: serverId,
			body: hotModuleCache
		  });
...
var reload = function(agent, msg, cb) {
    var sid, record;
    var serverInfo = {};
    var count = utils.size(agent.idMap);
    var latch = countDownLatch.createCountDownLatch(count, {timeout: Constants.TIME.TIME_WAIT_COUNTDOWN}, function() {
    ...
    for(sid in agent.idMap) {
        record = agent.idMap[sid];
        agent.request(record.id, module.exports.moduleId, { signal: msg.signal, hotFile: msg.hotFile }, callback);
    }
};
